...
function RankingTable({ ranking, fontSize, startTime }) {
  ...
  const topScale = [1, 0.9, 0.8, 0.75];
  const topColor = ["#FF004E", "#FF00C3", "#9D65FF", "#3D50FF"];
  return (
      ...
      <Column style={{ width: "100%" }}>
        {top.slice(0, 3).map((d, i) => {
          return (
            ...
            >
              {i + 1}位 {d.name}さん{" "}
              {resultTimeStr(d.time.getTime() - startTime.getTime())}
            </p>
          );
        })}
      </Column>
