...
class QuizPage extends Component {
  ...
  openPageHandller = dx => {
    return () => {
      if (typeof this.props.pages !== "undefined") {
        if (
          this.props.pageNumber + dx >= 0 &&
          this.props.pageNumber + dx < this.props.pages.length
        ) {
  ...
  renderMessage({ title, fontScale, messages, links }) {
    return (
      <Message
        fontSize={this.props.fontSize}
        title={title}
  ...
  renderResult({ title, messages, links, options }) {
    return (
      <Result
        fontSize={this.props.fontSize}
        time={this.props.time}
  ...
  renderEntry({ title, messages, links, entry }) {
    return (
      <Entry
        fontSize={this.props.fontSize}
        title={title}
...
export default connect(
  ...
  dispatch => ({
    ...
    saveImageMap: (filename, imageMap, callback) =>
      dispatch(saveImageMap(filename, imageMap, callback)),
  })
)(QuizPage);
