...
class Text extends Component {
  ...
  resize() {
    this.setState({
      startResize: true,
      fontSize: fontSize({ width: window.innerWidth, height: window.innerHeight }),
    }, () => {
      const fontScale = this.state.fontScale;
      const fontSize = this.state.fontSize*this.state.fontScale;
      const offsetHeight = this.valueText.offsetHeight+this.commentText.offsetHeight;
  ...
  render() {
    const fontSize = this.state.fontSize*this.state.fontScale;
    const height = (this.state.fontHeight<this.state.minHeight)?this.state.minHeight:this.state.fontHeight;
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="100%" height={this.props.maxHeight}>
        {
          <foreignObject x="0" y={ (this.state.startResize)?height:0 } width="100%" height="100%">
            <div ref={ t => this.valueText = t } style={{ width: "100%", textAlign, fontSize, }}><MathJax value={ this.props.value }/></div>
            <div ref={ t => this.commentText = t } style={{ width: "100%", textAlign, fontSize: fontSize*0.7, }}><MathJax value={ this.props.comment }/></div>
          </foreignObject>
        }
      </svg>
    )
